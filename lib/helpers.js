...
module.exports = {
  ...
  convertSwaggerRequestToItem: function (swaggerData, pathItem) {
    ...
    for (param in thisParams) {
      if (thisParams.hasOwnProperty(param) && thisParams[param]) {
        ...
        if (thisParams[param].hasOwnProperty('enum') && thisParams[param].enum.length == 1) {
          defaultVal = thisParams[param].enum[0];
        }
        else if (thisParams[param].hasOwnProperty('default')) {
        ...
        if (thisParams[param].in === 'query') { // && this.options.includeQueryParams !== false) {
          ...
          if (!thisParams[param].required) {
            disabledQueryParams.push(thisParams[param].name);
          }
        }

        else if (thisParams[param].in === 'header') {
          if (thisParams[param].name !== 'Authorization') {
            rHeaders += thisParams[param].name + ': ' + defaultVal + '\n';
          }
          if (!thisParams[param].required) {
            disabledHeaders.push(thisParams[param].name);
          }
        }
          ...
          if (thisParams[param].schema.hasOwnProperty('example')) {
            rData = thisParams[param].schema.example;
          }
          else {
            try {
          ...
          rBodySchema = thisParams[param].schema;
        }
          ...
          rData.push({
            ...
            'value': defaultVal,
            'type': 'text',
            'disabled': !thisParams[param].required
          });
        }
    ...
    request.url.query.each(queryParam => {
      if (disabledQueryParams.includes(queryParam.key)) {
        queryParam.disabled = true;
      }
    })
    request.forEachHeader(header => {
      if (disabledHeaders.includes(header.key)) {
    ...
    // Add a "response" to capture all request body fields
    if (Object.keys(rBodySchema).length > 0) {
      this.addResponseForAllRequestBodyFields(rBodySchema, item);
    }
        ...
        for (exampleMimeType in swaggerResponse.examples) {
          if (swaggerResponse.examples.hasOwnProperty(exampleMimeType)) {
            ...
            if (typeof example === 'object') {
              postmanResponse.body = JSON.stringify(example, null, 2);
            }
            else {
            ...
            if (resCode !== 'default') {
              ...
              postmanResponse.name = `${postmanResponse.code} response`;
            }
            else {
              postmanResponse.name = 'Default response';
            }
  ...
  // Adds a "response" to capture all request body fields
  addResponseForAllRequestBodyFields: function (schema, item) {
    var requestBody = JSON.stringify(this.getAllRequestBodyFields(schema), null, 2);
    var postmanResponse = new Response();
    postmanResponse.name = 'All request body fields';
    postmanResponse.originalRequest = {
  ...
  getAllRequestBodyFields: function(schema) {
    var requestBody;
    // Schema uses the allOf construction to join multiple objects together
    if (schema.hasOwnProperty('allOf')) {
      requestBody = {};
      schema.allOf.forEach(entry => {
        if (entry.hasOwnProperty('properties')) {
          Object.keys(entry.properties).forEach(property => {
            requestBody[property] = this.getAllRequestBodyFields(entry.properties[property]);
          });
        }
        if (entry.hasOwnProperty('additionalProperties')) {
          requestBody['*'] = this.getAllRequestBodyFields(entry.additionalProperties);
        }
      });
    }
    // Schema has no defined type
        ...
        case 'object':
          ...
          if (schema.hasOwnProperty('properties')) {
            Object.keys(schema.properties).forEach(property => {
              requestBody[property] = this.getAllRequestBodyFields(schema.properties[property]);
            });
          }
          if (
            ...
            requestBody['*'] = this.getAllRequestBodyFields({});
          }
          else if (
            schema.hasOwnProperty('additionalProperties') &&
            typeof schema.additionalProperties != 'boolean'
          ) {
            requestBody['*'] = this.getAllRequestBodyFields(schema.additionalProperties);
          }
          break;
        // Schema is an array
        case 'array':
          requestBody = [this.getAllRequestBodyFields(schema.items)];
          break;
        // Schema is a scalar
        default:
          if (schema.hasOwnProperty('format')) {
            requestBody = `${schema.format} ${schema.type}`;
          }
          else {
